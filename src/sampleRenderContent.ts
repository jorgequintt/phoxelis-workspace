...
export function sampleRenderContent(phoxelis: ReturnType<typeof Phoxelis>, rows: number, cols: number) {
...
function renderPhoxelBack(
  ...
  if (!takenCells.has(`${r};${c}`)) {
    phoxelis.renderPhoxel(char, fg, bg, r, c);
  }
}
function fill(color: string) {
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      phoxelis.renderPhoxel(' ', color, color, r, c);
    }
  }
}
function write(x: number, y: number, text: string, maxWidth: number = 70) {
  ...
  text.split('').forEach((char) => {
    ...
    phoxelis.renderPhoxel(char, colors.white, colors.blue, currentRow, x + charCount);
